...
length tables. When the position is unambiguously within one build's range
but beyond the other's, we can short-circuit the upstream call with a
structured build_mismatch envelope. Ambiguous positions (within both builds)
return None and the call proceeds.
"""
...
def likely_build(chrom: str, pos: int) -> str | None:
    ...
    build-stable and the position-length heuristic does not apply.
    """
    c = _strip_chr(chrom)
    ...
    # Position valid only on the longer of the two builds.
...
def dataset_build(dataset: str) -> str:
    """Map a gnomAD dataset name to its reference build name."""
    return "GRCh37" if dataset == "gnomad_r2_1" else "GRCh38"


def detect_variant_id_mismatch(variant_id: str, dataset: str) -> str | None:
    """Return the inferred build when variant_id's coordinate clearly belongs to a different build, else None."""
    try:
    ...
    if inferred and inferred != dataset_build(dataset):
        return inferred
    return None


def detect_region_mismatch(chrom: str, start: int, dataset: str) -> str | None:
    """Same logic as detect_variant_id_mismatch but for region (chrom, start) pre-split."""
    inferred = likely_build(chrom, start)
    if inferred and inferred != dataset_build(dataset):
        return inferred
    return None
