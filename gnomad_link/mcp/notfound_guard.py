...
* Layer 2 -- ``on_read_resource`` boundary: an unknown (URL-valid) resource makes
  ...
  URI-free ``ResourceError``. gnomAD's resource handlers raise no author-authored
  ``ResourceError`` (they only return payloads), so every exception here is a
  core not-found/read failure and is replaced unconditionally.
...
(``errors.py``). Layer 6 (OTel span redaction) is a no-op here: gnomAD-link
declares no ``opentelemetry-sdk`` dependency, and the active tracer provider is
the API's non-recording ``ProxyTracerProvider`` (spans never record, so no span
exception attributes are ever captured) -- there is nothing to redact (fleet
policy: do NOT add the SDK dependency).

Gnomad-specific invariant: ``get_diagnostics`` returns two process-global error
rings to any caller, so a caller-supplied name must NEVER reach them. The Layer-1
preflight and the log scrub therefore NEVER call ``record_mcp_error`` /
``record_schema_drift`` with the requested name (they log a fixed marker only),
keeping the hostile name out of the cross-session diagnostics ring.
"""
...
from gnomad_link.mcp.errors import _provenance_meta
...
def unknown_tool_envelope() -> dict[str, Any]:
    ...
    return {
        ...
        "fallback_tool": _UNKNOWN_TOOL_FALLBACK,
        "fallback_args": {},
        "recovery": _UNKNOWN_TOOL_RECOVERY,
        "_meta": {
            "next_commands": [{"tool": _UNKNOWN_TOOL_FALLBACK, "arguments": {}}],
            **_provenance_meta(),
        },
    }
...
class NotFoundGuard(Middleware):
    ...
    async def on_read_resource(
        ...
        so the URI never reaches the caller/protocol. gnomAD raises no
...
def _is_structured_envelope(call_result: mcp.types.CallToolResult) -> bool:
    ...
    Distinguishes a structured gnomad-link error (already input-free -- it carries
    an ``error_code``; e.g. the ``output_validation_failed`` envelope) from a RAW
    FastMCP dispatch error whose plain-text message echoes the caller-supplied tool
...
def install_protocol_error_handler(mcp_server: Any) -> None:
    ...
    exist) and AFTER any other request-handler wrappers (e.g. the output-schema
    handler) so this is the outermost wrapper on the raw handlers.
    """
...
class _NotFoundLogScrubFilter(logging.Filter):
    ...
    is still emitted for operational visibility. The scrubbed record carries no
    caller input, so it never feeds the ``get_diagnostics`` rings either.
    """
