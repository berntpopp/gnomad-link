...
`gnomad-link` is a thin FastMCP backend in the GeneFoundry fleet. It is unauthenticated
...
```bash
gh api -X PATCH repos/berntpopp/gnomad-link \
...
```bash
gh api repos/berntpopp/gnomad-link --jq '.security_and_analysis'
```
