"""Contract Truth v1 gate against the live gnomAD MCP registry."""
...
async def test_documentation_matches_live_mcp_contract(
    ...
    monkeypatch.delenv("GNOMAD_LINK_MCP_PROFILE", raising=False)
    monkeypatch.chdir(tmp_path)

    from gnomad_link.mcp.facade import create_gnomad_mcp

    def fake_service_factory() -> None:
        raise AssertionError("contract discovery must not invoke the gnomAD service")

    tools = await create_gnomad_mcp(service_factory=fake_service_factory).list_tools()
